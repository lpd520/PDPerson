...
NSString *const PathSpace =@"JHKit";
...
+ (JHCacheManager *)sharedInstance{
    static JHCacheManager *cacheInstance=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cacheInstance = [[JHCacheManager alloc] init];
    });
...
- (id)init{
    ...
    if (self) {
        ...
        _operationQueue = dispatch_queue_create("com.dispatch.JHCacheManager", DISPATCH_QUEUE_SERIAL);
...
- (void)initCachesfileWithName:(NSString *)name{
    ...
    self.diskCachePath =[[self JHKitPath] stringByAppendingPathComponent:name];
