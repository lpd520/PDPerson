...
- (void)initDataSource
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        MSSCalendarManager *manager = [[MSSCalendarManager alloc]initWithShowChineseHoliday:self.showChineseHoliday showChineseCalendar:self.showChineseCalendar startDate:self.startDate];
...
- (void)addWeakView
{
    UIView *weekView = [[UIView alloc]initWithFrame:CGRectMake(0, JHTopHeight, MSS_SCREEN_WIDTH, MSS_WeekViewHeight)];
    weekView.backgroundColor = JHViewBgColor;
...
- (void)createUI
{
    UIView *nav = [[UIView alloc] initWithFrame:CGRectMake(0, 0, MSS_SCREEN_WIDTH, JHTopHeight)];
    UIButton *back = [UIButton buttonWithType:UIButtonTypeCustom];
    [back setImage:[UIImage imageNamed:@"nav_back_white_item"] forState:UIControlStateNormal];
    [back addTarget:self action:@selector(backBtnClicked) forControlEvents:UIControlEventTouchUpInside];
//    back.frame = CGRectMake(6, 20, 44, 44);
    [nav addSubview:back];
    nav.backgroundColor = JHNavBgColor;
...
//    UILabel *titleLb = [[UILabel alloc] initWithFrame:CGRectMake(0, JHTopHeight - 44, 150, 44)];
    UILabel *titleLb = [[UILabel alloc] init];
    titleLb.text = @"出发日期选择";
    titleLb.font = [UIFont systemFontOfSize:17];
    titleLb.textColor = [UIColor whiteColor];
    [nav addSubview:titleLb];
//    titleLb.center = CGPointMake(nav.jh_width / 2, titleLb.center.y);
    ...
    _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, JHTopHeight + MSS_WeekViewHeight, width * 7, MSS_SCREEN_HEIGHT - 64 - MSS_WeekViewHeight) collectionViewLayout:flowLayout];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell)
    {
        ...
        if (_currentDateInterval == calendarItem.dateInterval) {
//            cell.isSelected = YES;
            cell.selected = YES;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.currentDateInterval = calendaItem.dateInterval;
    if ([_delegate respondsToSelector:@selector(calendarViewConfirmClickWithSelectedDate:)]) {
        [_delegate calendarViewConfirmClickWithSelectedDate:calendaItem];
    }
    [self.navigationController popViewControllerAnimated:YES];
//    MSSCalendarCollectionViewCell *cell = (MSSCalendarCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
//    cell.isSelected = YES;
//    cell.dateLabel.textColor = MSS_SelectTextColor;
//    // 当开始日期为空时
//    if(_startDate == 0)
