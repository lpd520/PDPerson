...
//新绑定QQ
- (NSURLSessionDataTask *)bindWithQQ:(nonnull NSString *)openId
                                nickname:(nonnull NSString *)nickname
                                 success:(nonnull JHNetworkRequestSuccessVoid)success
                                 failure:(nonnull JHNetworkRequestFailure)failure;

//验证第三方登录信息
- (NSURLSessionTask *)checkSocialLoginWithQQ_open_id:(nullable NSString *)qq_open_id we_chat_open_id:(nullable NSString *)we_chat_open_id unionId:(nonnull NSString *)unionId success:(void(^)(YCSocialAccountInfo *info))success failure:(nonnull JHNetworkRequestFailure)failure;

//验证手机是否注册
- (NSURLSessionTask *)checkMobile:(NSString *)mobile withSuccess:(void(^)(YCPhoneNumInfo *info))success failure:(nonnull JHNetworkRequestFailure)failure;
...
/**
 ...
 @param parameters
    status string 0=>所有 1=>未读 2=>已读
 @param success
    messages 消息模型数组，total消息总条数
 */
- (NSURLSessionDataTask *)getMyMessageWithParameters:(NSDictionary *)parameters
                                             success:(void (^)(NSArray *messages, NSUInteger total))success
                                             failure:(JHNetworkRequestFailure)failure;
...
/**
 ...
 @param msgId 消息ID
 */
- (NSURLSessionDataTask *)updateReadMessageWithId:(NSUInteger)msgId
                                          success:(JHNetworkRequestSuccessVoid)success
                                          failure:(JHNetworkRequestFailure)failure;
@end
