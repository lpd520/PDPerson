...
@property (nonatomic , weak , readwrite) UIImageView * navbarBottomLine;
...
+ (void) _setupNavigationBarTheme{
    ...
    // 设置导航栏的背景渲染色
    [appearance setBarTintColor:mainThemeRed];
    // 设置导航栏的背景渲染色或背景图片
...
- (void)_setupNavigationBarBottomLine{
    ...
    UIImageView *navSystemLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.navigationBar.frame.size.height - navSystemLineH, SCREEN_W, navSystemLineH)];
    
    navSystemLine.backgroundColor = [UIColor clearColor];///  搞个扎眼睛的颜色
    [self.navigationBar addSubview:navSystemLine];
    
    self.navbarBottomLine = navSystemLine;
...
- (void)showNavigationBottomLine {
    ...
    self.navbarBottomLine.hidden = NO;
}
/// 隐藏导航栏的细线
- (void)hideNavigationBottomLine {
    
    self.navbarBottomLine.hidden = YES;
}
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if (self.viewControllers.count > 0){
        ...
        //        viewController.navigationItem.leftBarButtonItem = [ mh_backItemWithTitle:@"返回" imageName:@"barbuttonicon_back_15x30" target:self action:@selector(_backItemDidClicked)];
    }
    // push
